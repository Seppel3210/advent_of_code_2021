...
fn main() {
    ...
    let mut last_winnig_card_id = None;
    let mut last_winnig_num = 0;
    for number in numbers {
        for &num_to_cross in &index[&number] {
            let NumPosition { card_id, x, y } = num_to_cross;
            if !already_won.contains(&card_id) && cards[card_id].check_bingo(x, y) {
                last_winnig_card_id = Some(card_id);
                last_winnig_num = number;
                already_won.insert(card_id);
            }
        }
    }

    for row in &cards[last_winnig_card_id.unwrap()].numbers {
        for num in row {
    ...
    let value: u32 = cards[last_winnig_card_id.unwrap()]
        .numbers
        .iter()
        .flat_map(|row| row)
        ...
        .sum();
    println!("solution: {}", last_winnig_num as u32 * value);
}
